...
Semantic html is a way of writing html code thats readable and gives meaning to the webpage.
2. Name two big differences between ```display: block;``` and ```display: inline;```.
display:block means that the element is displayes as a block, as paragraphs and headers have always been and display:inline means that the element is displayed inside the current block on the same line. 
3. What are the 4 areas of the box model?
content, padding, border, and the margin
4. While using flexbox, what axis does the following property work on: ```align-items: center```?
the cross axis
5. Explain why git is valuable to a team of developers.
any team member can make adjustemnts on a file without ruining someone elses work or a companies work piece.
