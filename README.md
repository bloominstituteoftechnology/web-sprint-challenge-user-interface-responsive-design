...
++Semantics is used to give meaning to both the browser and the developer.
++helps keep the developer organized, and in the case of people using screen
++readers, gives them a way to jump from section to section and know what its about.

2. Name two big differences between ```display: block;``` and ```display: inline;```.
++Display block will do exactly what it says, make a block with margins, where as INLINE does not have top or bottom margins
...
+++It allows many users to collaborate on different parts of a web page, be it elements, features, or content, at the same time. Then, the project lead can merge them together if they fit within the structure.
also, it keeps track of different builds, so if you break something with a new feature, you can go back to a working version.
